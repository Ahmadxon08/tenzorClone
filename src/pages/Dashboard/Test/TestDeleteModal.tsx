...
interface TestDeleteModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: () => void;
  testTitle?: string;
...
const TestDeleteModal: React.FC<TestDeleteModalProps> = ({
  open,
  onClose,
  onConfirm,
  testTitle,
      ...
      <DialogContent className="sm:max-w-[400px] bg-gradient-to-br from-[#0a1b30] to-[#071226] border border-white/10 text-white">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl font-bold text-red-400">
            <Trash2 size={20} /> {t("testsPage.deleteModal.title")}
          </DialogTitle>
        </DialogHeader>

        <p className="text-gray-300 mt-2 text-sm">
          {t("testsPage.deleteModal.message", { title: testTitle })}
        </p>
        ...
        <DialogFooter className="gap-3 sm:gap-2 pt-5">
          ...
          >
            {t("testsPage.deleteModal.cancel")}
          </button>
          ...
          >
            {isLoading
              ? t("testsPage.deleteModal.deleting")
              : t("testsPage.deleteModal.delete")}
          </button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
