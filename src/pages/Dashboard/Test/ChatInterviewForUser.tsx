...
export default function ChatInterview() {
  ...
  const sessionIdFromParams = params.get("session_id");
  ...
  const [inviteTimer, setInviteTimer] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const fetchSession = async () => {
      ...
      try {
        const resp = await fetch(
          `https://ai.tenzorsoft.uz/test-invite/${token}`,
          {
            method: "GET",
          }
        );
        ...
        if (!resp.ok) {
          if (data?.detail?.message && data.detail.remaining) {
            const { days, hours, minutes, seconds } = data.detail.remaining;
            setInviteError({
            ...
            throw new Error(data?.detail?.message || "Session fetch failed");
          }
    ...
    void fetchSession();
  }, [sessionIdFromParams, token]);

  useEffect(() => {
    if (!inviteTimer || inviteTimer <= 0) return;

    const interval = setInterval(() => {
      setInviteTimer((prev) => Math.max(0, prev - 1));
    }, 1000);

    return () => clearInterval(interval);
  }, [inviteTimer]);

  const formatInviteTime = (seconds: number) => {
  ...
  const postChatMessage = useCallback(
    async (message: string) => {
      ...
      return apiService.chatTest(body, token);
    },
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const resp = await apiService.getTestInvite(token);
      const payload = resp;
      ...
      if (selectedTestId && payload.test.id !== selectedTestId) {
        setError("Tanlangan test mos kelmadi.");
        setSelected(null);
        return;
      }

      setSelected(payload.test);
    } catch (e: unknown) {
      setError(getErrorMessage(e) || "Load test error");
      setSelected(null);
    }
  ...
  const handleAcceptRules = useCallback(async () => {
    if (fsBtnBusy || loadingSession) return;
    setFsError("");
    setFsBtnBusy(true);
    try {
      await requestFS();
  ...
  const triggerInitialChat = useCallback(
    async (chosenLang: "uz" | "ru" | "en") => {
      ...
      try {
        ...
        const resp = await apiService.chatTest(body, token!);
  ...
  const sendMessage = async () => {
    ...
    setBusy(true);
    setError("");
    console.log(userMsg, "this si ms");
    try {
      console.log("error here");
      const resp = await postChatMessage(userMsg);
      console.log("error here", resp);
      let ans =
  ...
  return (
    <>
      ...
      {inviteError && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-transparent backdrop-blur-2xl px-6 transition-all duration-500 font-sans">
          <div className="w-full max-w-3xl text-center space-y-12 animate-in fade-in zoom-in duration-300">
            {inviteTimer > 0 ? (
              <>
                ...
                <div className="flex items-center justify-center gap-3 md:gap-6">
                  <TimeCard
                    value={formatInviteTime(inviteTimer).split(":")[0]}
                    label="hours"
                  />
                  <span className="text-5xl font-bold text-white/50 self-start mt-4">
                    :
                  </span>
                  <TimeCard
                    value={formatInviteTime(inviteTimer).split(":")[1]}
                    label="minutes"
                  />
                  <span className="text-5xl font-bold text-white/50 self-start mt-4">
                    :
                  ...
                  <TimeCard
                    value={formatInviteTime(inviteTimer).split(":")[2]}
                    label="seconds"
                  />
                </div>
                ...
                <div className="space-y-4 max-w-md mx-auto">
                  <p className="text-xl font-medium text-white/90">
                    Before the start of your meeting
                  </p>
                  <p className="text-lg text-white/80 leading-snug italic">
                    Meanwhile have some tea and a book to keep you busy
                  </p>
                </div>
              </>
            ) : (
              <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-700">
                <div className="space-y-4">
                 ...
                 Sizning suhbat vaqtingiz boshlandi.
                  </p>
                </div>
            ...
            {/* Decorative lines from your reference image */}
...
export const TimeCard = ({ value, label }: { value: string; label: string }) => (
  <div className="flex flex-col gap-3">
    <div className="flex gap-1.5">
      {value.split("").map((char, i) => (
        <div
          key={i}
          className="w-14 h-20 md:w-20 md:h-28 flex items-center justify-center rounded-2xl bg-white shadow-[0_8px_0_0_#cbd5e1] text-slate-900 text-4xl md:text-5xl font-black"
        >
          {char}
        </div>
      ))}
