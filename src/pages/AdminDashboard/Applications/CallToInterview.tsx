...
const interviewFormSchema = z.object({
  ...
  description: z.string().max(500, "Maksimal 500 belgi ruxsat etiladi").optional(),
...
const CallToInterview: React.FC<CallToInterviewProps> = ({
  ...
  const publicKey = config?.publicKey ?? config?.public_key ?? "JhwH4LrDLnVgQ3GC";
  ...
  const isFormValidWithLocation =
    ...
    Boolean(watch("interviewDate")?.trim());
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      ...
      >
        ...
        {/* CONTENT */}
        <form onSubmit={handleSubmit(handleSendSMS)} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
            ...
            <div className="p-4 border border-white/10 rounded-lg bg-[#0a1b30]/60 space-y-5">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">
                  {t("applicationsPage.call.position")} <span className="text-red-500">*</span>
                </label>
                <Controller
                  name="title"
                  control={control}
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">
                  {t("applicationsPage.call.date")} <span className="text-red-500">*</span>
                </label>
                <Controller
                  name="interviewDate"
                  control={control}
            ...
            <div className="p-4 border border-white/10 rounded-lg bg-[#0a1b30]/60 space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <p className="text-white"><span className="text-gray-400">Kompaniya:</span> <span className="font-semibold text-blue-300">{siteName || "-"}</span></p>
              </div>
            </div>
          ...
          <DialogFooter className="border-t border-white/10 px-6 py-4 flex gap-3 bg-[#0a1b30]/40 flex-shrink-0">
            ...
            `}
            >
              {isLoading ? "Yuborilmoqda..." : t("applicationsPage.call.submit")}
            </button>
          </DialogFooter>
        </form>
      </DialogContent>
