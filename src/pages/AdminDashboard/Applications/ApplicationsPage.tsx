...
const PASS_THRESHOLD = 60; // O‘tish chegarasi (kerak bo‘lsa o‘zgartiring)

type ScoreFilter = "all" | "80plus" | "60to79" | "40to59" | "lt40";
...
const ApplicationsPage: React.FC = () => {
  ...
  // Yuklab olish holati (fayl nomi bo‘yicha)
  ...
  // Tanlangan sayt (default: birinchi sayt)
  const [selectedSite, setSelectedSite] = useState<Site | null>(
    user?.sites?.[0] ?? ({} as Site)
  );

  // ——— Config: download URL bazasi (kerak bo‘lsa o‘zingiznikiga almashtiring) ———
  // Masalan: "https://vakansiya.my-blog.uz/download/"
  const downloadBaseUrl = "https://ai.tenzorsoft.uz/download/";

  // buildUrlWithAuth – so‘rovga query qo‘shish (public_key, username)
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      // fetch URLni limit, offset, va site info bilan yasash
      ...
      const apps = (data.data || []).map((item: any) => ({
        id: item.id,
        name: item.name,
  ...
  const fetchAcceptedApplications = async () => {
    try {
      ...
      const offset = (currentPage - 1) * limit;
      const baseUrl = "https://ai.tenzorsoft.uz/vakand/list";
      const url = `${baseUrl}?type=widget&has_invited&limit=${limit}&offset=${offset}`;
      ...
      const accepted = (data.data as Application[]).filter(
        (item) => item.invited_time
      );
      ...
      setAcceptedApplications(sorted);
      setTotalAccepted(sorted.length);
    } catch (err) {
  ...
  // Har safar sahifa yoki limit o‘zgarsa — yangi data yuklanadi
  useEffect(() => {
    fetchApplications();
  }, [currentPage, limit, selectedSite]);
    ...
    return applications
      .filter((app) => {
        ...
        const matchesSearch =
          ...
          (app.vakansiya_title?.toLowerCase().includes(q) ?? false);
        ...
        else if (scoreFilter === "40to59")
          matchesScore = app.score >= 40 && app.score < 59;
        else if (scoreFilter === "lt40") matchesScore = app.score < 40;

        if (!matchesScore) return false;

        // natija (o‘tgan / o‘tolmagan)
  ...
  }, [applications, searchTerm, scoreFilter, resultFilter]);
    ...
    return acceptedApplications
      .filter((app) => {
        ...
        if (scoreFilter === "80plus") matchesScore = app.score >= 80;
          ...
          matchesScore = app.score >= 40 && app.score < 60; // 59 emas 60
        else if (scoreFilter === "lt40") matchesScore = app.score < 40;
  ...
  const filterByDate = async () => {
    ...
    if (!fromDate && !toDate) {
      return; // alert o‘rniga return qilamiz
    }
  ...
  console.log(applications, "this is shitty");
  ...
  const openCallModal = (app: Application) => {
    ...
    markAsViewedMutation.mutate(app.id);
  };
      ...
      <Tabs className="mt-6">
        ...
        <TabPanel>
          <div className="bg-gradient-to-br from-[#0f2a43]/60 to-[#0a1b30]/60 backdrop-blur-sm border border-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 shadow-xl overflow-x-auto">
                  ...
                  <Select
                    ...
                    onValueChange={(val: string) => {
                      try {
                        ...
                        if (apiService.setSite) apiService.setSite(null, null);
                      }
                    }}
                  >
            ...
            >
              <div className="flex flex-wrap items-center gap-2 p-3 rounded-xl border border-white/10 bg-[#0a1b30]/60">
                ...
                <div className="w-full sm:w-auto min-w-[120px]">
                  ...
                  >
                    <SelectTrigger className="h-10 rounded-xl bg-[#0a1b30] border border-white/10 text-white text-sm w-[120px]">
                      <SelectValue
                        placeholder={
                    ...
                    <SelectContent className="bg-[#0a1b30] border border-white/10 text-white">
                      ...
                      <SelectItem value="40to59">40 – 59</SelectItem>
                      <SelectItem value="lt40">&lt; 40</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <div className="flex items-center gap-1 min-w-[130px]">
                    ...
                    <Input
                      ...
                      className="h-10 rounded-xl bg-[#0a1b30] border border-white/10 text-white px-3 text-sm w-[120px]"
                    />
                  </div>
            ...
            <div className="antd-dark-table mt-1">
              <Table
                className="antd-dark-table min-w-100"
